...
func (uh *userHandlers) RegisterUser(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		uh.logger.Error("error on decoding input")
	...
	if err != nil {
		uh.logger.Error("error on hashing password")
	...
	err = uh.database.Set(fmt.Sprintf("user_email:%s", input.Email), input.Id)
	if err != nil {
		uh.logger.Error("error on creating user_email index", "error", err)
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}

	err = uh.database.Insert(fmt.Sprintf("user:%s", id), map[string]interface{}{
		"id":       input.Id,
		"email":    input.Email,
		"password": input.Password,
	})
	if err != nil {
		uh.logger.Error("error on registering user", "error", err)
		http.Error(w, http.StatusText(http.StatusInternalServerError), http.StatusInternalServerError)
		return
	}
